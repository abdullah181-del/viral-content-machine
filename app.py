...
def scheduled_post_job(slot: dict):
    ...
    # Post with image
    image_bytes = generate_and_download_image(slot["niche"], result["text"], api_key)
    if image_bytes:
        post_result = post_image_to_facebook(page_id, page_token, image_bytes, result["text"])
    else:
        post_result = {"success": False, "error": "Image generation failed"}

    # Fallback to text-only if image generation/upload fails
...
def load_config() -> dict:
    if os.path.exists(CONFIG_FILE):
        with open(CONFIG_FILE, "r") as f:
            return json.load(f)
    return {"page_token": "", "page_id": "", "groq_api_key": ""}
...
def get_api_key(cfg: dict) -> str:
    """Get Groq API key — Streamlit Secrets (cloud) first, then local config.

    GEMINI_API_KEY is still accepted as a legacy fallback so existing
    Streamlit deployments do not break after upgrading.
    """
    try:
        ...
        pass
    return cfg.get("groq_api_key") or cfg.get("gemini_api_key", "")
    ...
    if "variations" in st.session_state:
        ...
        def render_variation_tab(tab_obj, var_key, css_class, label):
            with tab_obj:
                ...
                if text:
                    sections = {key: escape(value) for key, value in parse_post_sections(text).items()}
                    st.markdown(f"""
                ...
                if pending:
                    ...
                    if p_img:
                        image_key = f"pending_image_{var_key}"
                        if image_key not in st.session_state:
                            with st.spinner("🖼️ Image generate ho rahi hai..."):
                                st.session_state[image_key] = generate_and_download_image(p_niche, p_text, get_api_key(cfg))
                        img_bytes = st.session_state.get(image_key)
                        st.markdown("**🖼️ Image Preview — approve karey ya sirf text post karo:**")
                        if img_bytes:
                            st.image(img_bytes, use_container_width=True)
                        else:
                            st.warning("⚠️ Image generation failed — you can still post text only.")
                        st.caption("Auto-generated by Pollinations.ai · Free render + Groq prompt")
                        ...
                        img_bytes = None
                        st.markdown("**📝 Ready to publish text-only:**")
                        col_a, col_b = st.columns([2, 1])
                        with col_a:
                            do_text_post = st.button("📝 Confirm Text Post", key=f"do_txt_{var_key}", type="primary", use_container_width=True)
                        with col_b:
                            do_cancel = st.button("✖ Cancel", key=f"do_cancel_{var_key}", use_container_width=True)
                        do_img_post = False
                    ...
                    if do_img_post:
                        with st.spinner("📸 Image Facebook pe upload ho rahi hai..."):
                            fb_result = post_image_to_facebook(p_page_id, p_token, img_bytes, p_text) if img_bytes else {"success": False, "error": "No image was generated"}
                        ...
                        if fb_result.get("success"):
                            ...
                            st.session_state.pop(f"pending_post_{var_key}", None)
                            st.session_state.pop(f"pending_image_{var_key}", None)
...
with tab3:
    ...
    cloud_key = False
    try:
        if st.secrets.get("GROQ_API_KEY", "") or st.secrets.get("GEMINI_API_KEY", ""):
            cloud_key = True
    except Exception:
        pass

    if cloud_key:
        st.success("✅ Groq API key loaded from Streamlit Secrets (cloud deployment)")
    else:
        groq_key_input = st.text_input(
            "Groq API Key",
            value=cfg.get("groq_api_key") or cfg.get("gemini_api_key", ""),
            type="password",
            placeholder="gsk_..."
        )
    ...
    if st.button("💾 Save Settings", type="primary"):
        ...
        if not cloud_key:
            new_cfg["groq_api_key"] = groq_key_input
        else:
            new_cfg["groq_api_key"] = cfg.get("groq_api_key") or cfg.get("gemini_api_key", "")
        save_config(new_cfg)
        cfg = load_config()
        st.success("✅ Saved!")
...
with tab4:
    ...
    if not history:
        ...
        for item in reversed(history):
            niche_badge   = escape(str(item.get("niche") or item.get("type", "—")))
            hook_badge    = escape(str(item.get("hook_style", "")))
            var_badge     = escape(str(item.get("variation", "")))
            tone_val      = item.get("tone_level", "")
            tone_disp     = escape(f"Tone {tone_val}") if tone_val else ""
            post_id_disp  = escape(str(item.get('post_id', '—')))
            preview_text   = escape(str(item.get('text', ''))[:220])
            preview_suffix = '...' if len(str(item.get('text', ''))) > 220 else ''
